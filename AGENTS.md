...
- **Model Migrations & Versioning**: SwiftData models are managed under explicit `VersionedSchema` versions (`RunalystSchemaV1`, etc.) coordinated by `RunalystMigrationPlan`. When introducing breaking or structural changes, declare a new `VersionedSchema`, add it to `RunalystMigrationPlan.schemas`, and register the appropriate `MigrationStage` (`.lightweight` or `.custom`). For additive optional fields within an existing schema, mark properties as Optional (e.g., `String?`) and gracefully unwrap in code.
- **Zero Redundant Compute & Repairs**: Do not run ongoing data repair or migration loops (e.g., scanning all historical runs on app launch). Ingested workouts must calculate and finalize their working metrics and tags upon initial insertion.
- **Ingestion Performance & Scaling**: For $O(1)$ deduplication during HealthKit syncs, always use `Set<UUID>` (e.g., `Set(currentExistingRuns.map(\.hkWorkoutID))`) instead of $O(N \times M)$ `contains(where:)` array scans. Batch SQLite `try? modelContext.save()` commits (e.g., every 25 runs) instead of saving after every single run to prevent disk write bottlenecking on large historical archives.
- **User Action State Preservation**: State representing user actions (e.g., `DrillRecommendation.isCompleted` or `TrainingCorrection` feedback) must be immediately saved (`try? modelContext.save()`) upon user interaction and preserved across future schema migrations.
- **Sort Orders**: Fetch and process workouts newest-to-oldest (`$0.startDate > $1.startDate` / `HKSampleSortIdentifierStartDate` descending) so that the runner's most recent runs appear immediately on the dashboard and trigger high-priority AI coaching. Views like `DashboardView` present newest runs first via `@Query(sort: \.date, order: .reverse)`.
...
- **Dual-Scope Refresh Lifecycle**: Pull-to-refresh operates across two distinct architectural scopes:
  - *Macro Scope (`DashboardView`)*: Pull-to-refresh coordinates with `onSync(false)` inside an unstructured `Task`, re-queries recent global VO2 Max, recalculates baseline VO2 Max, invalidates `@AppStorage` headline caches (7-day, 30-day, all-time), and triggers macro readiness coaching via `fetchInsight()`.
  - *Micro Scope (`RunDetailView`)*: Pull-to-refresh invokes `HealthKitManager.shared.refreshWorkoutMetrics(for:in:)` to re-query the authentic `HKWorkout`, re-extract metrics via `extractRunRecord()`, and mutate SwiftData in-place while strictly preserving user `TrainingCorrection` manual overrides and `"userLinkedDrill"` / `"userUnlinkedDrill"` tags. Ad-hoc repair loops scanning historical runs in view `.task` modifiers are strictly forbidden.
