...
5. In your `Info.plist`, verify that the following keys are present with descriptive messages for the user:
   - `NSHealthShareUsageDescription` (e.g., "Runalyst needs to read your workout data to provide AI coaching insights.")
6. Select a compatible iOS Simulator or Physical Device (iOS 18+).
7. Build and Run (`Cmd + R`).

## Project Structure

- `Runalyst/Models/`: Contains the `SwiftData` schemas (`RunRecord`, `CoachingInsight`, `DrillRecommendation`).
...
- `Runalyst/Views/`: Minimalist SwiftUI components (Dashboard, Onboarding, RunDetail, Settings).
